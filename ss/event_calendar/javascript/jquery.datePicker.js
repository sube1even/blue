 ...
 **/
		...
		renderCalendar  :   function(s)
		{
			...
			if (s.showHeader != $.dpConst.SHOW_HEADER_NONE) {
				...
				// Insert the show week button
				headRow.append(
					jQuery(dc('th')).attr({'scope':'col', 'title':'Show week', 'class':'showweek'}).html('')
				);
			...
			while (w++<weeksToDraw) {
				...
				.attr('title',start_of_week + '/' + end_of_week)
				.hover(doHover, unHover);
					if (s.renderCallback) {
						s.renderCallback(q, currentDate, month, year);
					}
			r.append(q);
	...
	$.extend(
		...
		{	
			init : function(s)
			{
				...
				this.setRangeStart(s.rangeStart);
				this.setRangeEnd(s.rangeEnd);
			...
			setRangeStart : function(d)
			{
				if(d) {
					parts = d.split("-")
					this.rangeStart = Date.fromString(parts[2]+"/"+parts[1]+"/"+parts[0]);
				}
			},
			setRangeEnd : function (d)
			{
				if(d){
					parts = d.split("-")
					this.rangeEnd = Date.fromString(parts[2]+"/"+parts[1]+"/"+parts[0]);
				}
			},
							...
							.append(
								$('<h2></h2>')
									.bind(
										...
										function()
										{
											$(c.ele).trigger('monthSelected',[c,this]);
										}
									)
									.attr({
										title : 'Show entire month'
									}),
				...
				$td.bind(
					...
					function()
					{
						...
						if (!$this.is('.disabled')) {
							...
							if($this.attr('class').split(' ')[0].match('weekbutton')) {
								$(c.ele).trigger('weekSelected', [d, $td, s]);
							}						
							else {
